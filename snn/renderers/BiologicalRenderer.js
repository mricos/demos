...
class BiologicalRenderer {
  ...
  _drawActionPotential(ctx, modelState, startX, centerY, length) {
    ...
    // Only draw ions if there was an actual spike (not during integration)
    const hasSpike = modelState.lastSpike >= 0;
    if (hasSpike && timeSinceSpike >= 0 && timeSinceSpike < 100) {
