"""
Tau multitrack audio playback integration for ascii_scope_snn.

This module provides synchronized audio playback for the DAW by communicating
with the tau audio engine via Unix domain sockets.

Architecture:
    ascii_scope_snn lanes 1-8 → tau samples 1-8 → tau channels 0-3 → master
...
class TauMultitrack:
    """Direct socket communication with tau audio engine for multitrack playback."""

    def __init__(self, socket_path: str = "~/tau/runtime/tau.sock"):
        """
...
if __name__ == "__main__":
    ...
    if not tau.check_connection():
        print("Error: Tau daemon not running")
        print("Start with: tau start")
        exit(1)

    print("✓ Connected to tau")
