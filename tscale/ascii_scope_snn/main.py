#!/usr/bin/env python3
"""
ASCII Scope SNN - Multi-page oscilloscope for SNN kernel parameter tuning.
...
from .project import SNNProject
...
class App:
    ...
    def __init__(self, audio_path: str = None, project_dir: str = None):
        """
        ...
        """
        # Initialize project
        print("Initializing SNN project...")
        self.project = SNNProject(project_dir)
        print(f"Project: {self.project.project_dir}")
        print(f"CWD: {self.project.cwd_mgr.get_cwd()}")
        ...
        self.cli.add_output("ASCII Scope SNN - Type 'quickstart' for tutorial, 'help' for commands")
        self.cli.add_output("System ready. Lane 0 shows logs, Lane 9 shows events.", is_log=True, log_level="INFO")
        ...
        if local_config:
            print(f"Loaded local config from .snn/config")
    ...
    def save_state(self):
        """Save session state to .snn/session."""
        try:
            # Save session state
            session_data = {
            ...
            print("Session saved to .snn/session.json")
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ASCII Scope SNN - Audio Neural Network Parameter Tuning"
    )
    parser.add_argument(
        'audio',
        nargs='?',
    ...
    # Create app
    try:
        app = App(audio_path=args.audio, project_dir=args.project_dir)
    except Exception as e:
